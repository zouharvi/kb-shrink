...
data_q = np.array(read_pickle("computed/dpr-c-pruned-500.embd")
                  ["queries"])[:, [INDEX_0, INDEX_1]]
data_d = np.array(read_pickle("computed/dpr-c-pruned-500.embd")
                  ["docs"])[:, [INDEX_0, INDEX_1]]
...
plt.legend(
    ...
    [
        "$\\textbf{Queries}$", h_q.get_label(
        ), h_q_c.get_label(), h_q_cn.get_label(),
        "$\\textbf{Docs}$", h_d.get_label(
        ), h_d_c.get_label(), h_d_cn.get_label()
    ],
